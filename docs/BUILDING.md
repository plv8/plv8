...
This documentation is for Postgres installations that don't include plv8, i.e. `CREATE EXTENSION plv8` fails.

For technical support, please file a [github issue](https://github.com/plv8/plv8/issues) and we'll try to respond quickly.

# Adding plv8 to Cloud Hosted Postgres (as of 2019-02-25)
...
# Adding plv8 to Self-Hosted Postgres Instances (cloud, on-premise, laptops, etc)
...
## Using a pre-built/pre-compiled plv8 (recommended)
...
For best results, use [Docker multi-stage builds](https://www.google.com/search?q=docker+multi-stage+builds) to include just the files you need.  We've included an [example Dockerfile](/Dockerfile-example-use) that includes plv8 from dockerhub.

### pre-built plv8 for Docker - Alpine
...
## Building and installing plv8 from source

In addition to not having a choice, another reason to pre-build from source is to customize the build - see below for options.

note: plv8 has many large dependencies - 4+ cores and fast internet are recommended.

### Building and installing plv8 on Debian/Ubuntu Linux
...
```
$ make install  # note: may require root access, i.e. sudo make install
```
